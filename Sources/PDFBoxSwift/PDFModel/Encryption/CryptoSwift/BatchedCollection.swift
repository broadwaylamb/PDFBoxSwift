...
internal struct BatchedCollectionIndex<Base: Collection> {
  internal let range: Range<Base.Index>
}

extension BatchedCollectionIndex: Comparable {
  internal static func < <Base>(lhs: BatchedCollectionIndex<Base>,
...
struct BatchedCollection<Base: Collection>: Collection {
  ...
  func index(after idx: Index) -> Index {
    return Index(
      range: idx.range.upperBound..<nextBreak(after: idx.range.upperBound)
    )
  }
